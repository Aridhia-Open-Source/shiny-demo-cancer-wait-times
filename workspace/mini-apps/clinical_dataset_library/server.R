  ...
  observeEvent(input$sheet,{
    ...
    if (input$sheet != ""){
      ...
      list_of_sheets <- names(xl_sheets) %>%
        ...
        purrr::map(~read_rm_sheet(.x,input$sheet))%>% 
        purrr::map(add_ons_id)
      ...
      names(areas) <- c('West Midlands', 'South East Coast', 'London', 'North West', 'South Central', 'North East', 'Yorkshire and the Humber', 'East Midlands', 'East of England', 'South West', 'N/A')
      print(areas[['West Midlands']])
      dfs$regions <- areas
      ...
      b <- leaflet(options = leafletOptions(minZoom = 5.7, maxZoom = 5.7)) %>%
        addPolygons(data = map_gsdf, 
                    layerId= map_gsdf$scn16cd, 
                    color = "#444444", weight = 1, smoothFactor = 0.5,
                    popup = paste0("<b>",map_gsdf$scn16nm,"<br>", map_gsdf$average, "</b>"),
                    opacity = 1.0, fillOpacity = 0.5,
                    highlightOptions = highlightOptions(color = "white", weight = 2, bringToFront = TRUE))
